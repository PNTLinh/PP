...
class InputFeatures:
    """Class to represent processed features of an input example ready for model input."""
    def __init__(self, example_index, source_ids, target_ids, source_mask, target_mask):
...
def convert_examples_to_features(item):
    """
    Convert input example to model features.
    Expected item: (example, example_index, tokenizer, args, split_tag, task, sub_task, max_source_length, max_target_length)
    """
    try:
        (example, example_index, tokenizer, args, split_tag, task,
         sub_task, max_source_length, max_target_length) = item
    except ValueError:
        raise ValueError(f"Expected 9 elements in item, got {len(item)}: {item}")

    if task is None:
        raise ValueError("`task` is None. Please provide a valid task name.")

    # Prepare source and target depending on the task
    if task in ['summarize', 'translate', 'refine', 'concode', 'mbpp']:
        source_str = example.source
        target_str = example.target
    elif task == 'defect':
        source_str = example.source
        target_str = int(example.target)  # usually a binary label
    elif task == 'clone':
        source_str = example.source
        target_str = example.target
    else:
    ...
    # Tokenize source
    source_tokens = tokenizer.tokenize(source_str)[:max_source_length - 2]
    source_tokens = [tokenizer.cls_token] + source_tokens + [tokenizer.sep_token]
    source_ids = tokenizer.convert_tokens_to_ids(source_tokens)
    source_mask = [1] * len(source_ids)
    source_padding_length = max_source_length - len(source_ids)
    source_ids += [tokenizer.pad_token_id] * source_padding_length
    source_mask += [0] * source_padding_length
    ...
    if split_tag == 'test':
        ...
        if task == 'defect':
            ...
            target_tokens = tokenizer.tokenize(target_str)[:max_target_length - 2]
            target_tokens = [tokenizer.cls_token] + target_tokens + [tokenizer.sep_token]
            target_ids = tokenizer.convert_tokens_to_ids(target_tokens)
            target_mask = [1] * len(target_ids)
            target_padding_length = max_target_length - len(target_ids)
            target_ids += [tokenizer.pad_token_id] * target_padding_length
            target_mask += [0] * target_padding_length
...
def get_filenames(data_root, task, sub_task, split=''):
    if task == 'mbpp':
        # Dữ liệu MBPP luôn là 1 file .json cho mỗi split
        train_fn = os.path.join(data_root, 'mbpp_train.json')
        dev_fn = os.path.join(data_root, 'mbpp_dev.json')
        test_fn = os.path.join(data_root, 'mbpp_test.json')
    elif task == 'concode':
        if sub_task:  
...
class InputExample:
    """Class to represent a single data example with source and target."""
    def __init__(self, idx, source, target):
        self.idx = idx
        self.source = source
        self.target = target
# Sửa các hàm read_examples khác để nhận tham số args

def read_summarize_examples(filename, data_num, task, args=None):
    sub_task = None
    
    # Xác định sub_task từ stream trong args nếu có
    if args is not None and hasattr(args, 'stream') and args.stream.startswith('summarize_'):
        sub_task = args.stream.split('_')[1]  # e.g., 'ruby', 'python'
    else:
        # Nếu không có args.stream, thử xác định sub_task từ filename
        for lang in ['ruby', 'python', 'java', 'go']:
            if lang in filename:
                sub_task = lang
                break
    ...
    # Tải dữ liệu
    inputs, outputs, _ = get_data('summarize', 'train', sub_task=sub_task)
    
    # Tạo examples
    examples = [InputExample(i, inp, out) for i, (inp, out) in enumerate(zip(inputs, outputs))]
    
    # Giới hạn số lượng mẫu nếu cần
    return examples[:data_num] if data_num != -1 else examples

def read_mbpp_examples(filename, data_num, task, args=None):
    import json
    examples = []
    with open(filename, 'r', encoding='utf-8') as f:
        for i, line in enumerate(f):
            ...
            examples.append(InputExample(i, source, target))
    return examples[:data_num] if data_num != -1 else examples


def read_refine_examples(filename, data_num, task, args=None):
    sub_task = None
    
    # Xác định sub_task từ stream trong args nếu có
    if args is not None and hasattr(args, 'stream') and args.stream.startswith('refine_'):
        sub_task = args.stream.split('_')[1]  # e.g., 'small', 'medium'
    else:
        # Nếu không có args.stream, thử xác định sub_task từ filename
        if 'medium' in filename:
            sub_task = 'medium'
        else:
            sub_task = 'small'  # Mặc định
    
    # Tải dữ liệu
    inputs, outputs, _ = get_data('refine', 'train', sub_task=sub_task)
...
def read_concode_examples(filename, data_num, task, args=None):
    # Task concode không cần sub_task
    
    # Tải dữ liệu
    inputs, outputs, _ = get_data('concode', 'train')
    ...
    # Giới hạn số lượng mẫu nếu cần
    return examples[:data_num] if data_num != -1 else examples
def read_translate_examples(filename, data_num, task, args=None):
    sub_task = None
    
    # Xác định sub_task từ stream trong args nếu có
    if args is not None and hasattr(args, 'stream') and args.stream.startswith('translate_'):
        sub_task = args.stream.split('_')[1]  # e.g., 'java-cs'
    else:
        # Nếu không có args.stream, thử xác định sub_task từ filename
        if 'java-cs' in filename:
            sub_task = 'java-cs'
        elif 'cs-java' in filename:
            sub_task = 'cs-java'
    
    # Tải dữ liệu
    inputs, outputs, _ = get_data('translate', 'train', sub_task=sub_task)
    ...
    # Giới hạn số lượng mẫu nếu cần
    return examples[:data_num] if data_num != -1 else examples
def read_defect_examples(filename, data_num, task):
    inputs, outputs, _ = get_data('defect', 'train')
    # Đầu ra là nhãn nên vẫn ép kiểu str
    examples = [InputExample(i, inp, str(out)) for i, (inp, out) in enumerate(zip(inputs, outputs))]
    return examples[:data_num] if data_num != -1 else examples

def read_clone_examples(filename, data_num, task):
    inputs, outputs, _ = get_data('clone', 'train')
    # Với clone: inputs là danh sách các tuple (code1, code2)
    examples = [InputExample(i, inp1, inp2) for i, (inp1, inp2) in enumerate(zip(inputs, outputs))]
    return examples[:data_num] if data_num != -1 else examples

def read_examples(filename, data_num, task):
    """
    Đọc và tạo các ví dụ cho task cụ thể
    
    Args:
        filename (str): Đường dẫn đến tệp dữ liệu
        data_num (int): Số lượng ví dụ cần lấy (-1 để lấy tất cả)
        task (str): Loại task
    
    Returns:
        list: Danh sách các ví dụ đầu vào (InputExample)
    """
    read_example_dict = {
    ...
    # Kiểm tra xem task có trong danh sách hỗ trợ không
    if task not in read_example_dict:
        raise ValueError(f"Task không được hỗ trợ: {task}")
    
    return read_example_dict[task](filename, data_num, task)
...
def set_dist(args):
    # Ban đầu: args.n_gpu = torch.cuda.device_count()
    # Thay bằng dòng này để luôn có ít nhất 1 device:
    ...
    if args.local_rank != -1:
        ...
        # Nếu có GPU (n_gpu>0) thì dùng cuda, không thì dùng cpu
        args.device = torch.device("cuda" if args.n_gpu > 0 else "cpu")

def get_data(task, split, sub_task=None):
    """
    Hàm load dữ liệu cho các task khác nhau
    
    Args:
        task (str): Loại task ('translate', 'summarize', 'refine', 'concode', etc.)
        split (str): Phân vùng dữ liệu ('train', 'val', 'test')
        sub_task (str, optional): Sub-task cụ thể (ví dụ: 'ruby', 'python' cho summarize)
    
    Returns:
        tuple: (inputs, outputs, extra_info)
    """
    # Chuyển đổi split từ val -> validation cho HuggingFace
    hf_split = 'train' if split == 'train' else 'validation' if split == 'val' else 'test'
    
    # Xử lý theo từng loại task
    if task == 'translate':
        try:
            dataset_name = 'microsoft/code_x_glue_ct_code_to_code_trans'
            
            # Xác định cấu hình dựa trên task
            if sub_task:
                config = sub_task
            else:
                # Mặc định java-cs nếu không có sub_task
                config = 'java-cs'
            
            # Tải dataset từ HuggingFace Hub
            try:
                print(f"Đang tải dataset {dataset_name}/{config} cho split {hf_split}...")
                dataset = load_dataset(dataset_name, config, split=hf_split)
                
                # Trích xuất inputs và outputs dựa trên cấu hình
                if config == 'java-cs':
                    inputs = dataset['java']
                    outputs = dataset['cs']
                elif config == 'cs-java':
                    inputs = dataset['cs']
                    outputs = dataset['java']
                else:
                    raise ValueError(f"Không hỗ trợ cặp ngôn ngữ: {config}")
                
                print(f"Đã tải thành công {len(inputs)} mẫu cho task translate, cấu hình {config}")
                return inputs, outputs, None
                
            except Exception as e:
                print(f"Lỗi khi tải dataset từ HuggingFace Hub: {e}")
                
                # Fallback vào dữ liệu địa phương
                print("Đang thử tải từ dữ liệu địa phương...")
                data_dir = os.path.join(
                    os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),
                    "data", "code_translation", config
                )
                
                file_prefix = os.path.join(data_dir, split)
                
                # Đọc dữ liệu từ tệp địa phương
                if config == 'java-cs':
                    with open(f"{file_prefix}.java", 'r', encoding='utf-8') as f:
                        inputs = [line.strip() for line in f.readlines()]
                    with open(f"{file_prefix}.cs", 'r', encoding='utf-8') as f:
                ...
                print(f"Đã tải thành công {len(inputs)} mẫu từ dữ liệu địa phương")
                return inputs, outputs, None
                
        except Exception as e:
            raise ValueError(f"Không thể tải dữ liệu cho task translate: {e}")
    
    elif task == 'summarize':
        try:
            dataset_name = 'microsoft/code_x_glue_ct_code_to_text'
            
            # Xác định sub_task (ngôn ngữ)
            if not sub_task:
                sub_task = 'ruby'  # Mặc định là ruby nếu không chỉ định
            
            config = f'code_to_text_{sub_task}'
            
            try:
                print(f"Đang tải dataset {dataset_name}/{config} cho split {hf_split}...")
                # Thử tải từ HuggingFace
                dataset = load_dataset('xcodeparrot/code_x_glue_ct_code_to_text', config, split=hf_split)
                
                # Trích xuất inputs (code) và outputs (tóm tắt)
                if sub_task == 'ruby':
                    inputs = dataset['code']
                    outputs = dataset['docstring']
                elif sub_task == 'python':
                    inputs = dataset['code']
                    outputs = dataset['docstring']
                elif sub_task == 'java':
                    inputs = dataset['code']
                    ...
                    raise ValueError(f"Không hỗ trợ ngôn ngữ: {sub_task}")
                
                print(f"Đã tải thành công {len(inputs)} mẫu cho task summarize, ngôn ngữ {sub_task}")
                return inputs, outputs, None
                ...
                data_dir = os.path.join(
                    os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),
                    "data", "code_summarization", sub_task
                )
                
                file_prefix = os.path.join(data_dir, split)
                
                # Đọc dữ liệu từ tệp địa phương
                with open(f"{file_prefix}.code", 'r', encoding='utf-8') as f:
                    inputs = [line.strip() for line in f.readlines()]
                with open(f"{file_prefix}.docstring", 'r', encoding='utf-8') as f:
                    outputs = [line.strip() for line in f.readlines()]
            ...
            raise ValueError(f"Không thể tải dữ liệu cho task summarize: {e}")
    
    elif task == 'refine':
        try:
            # Mặc định là small nếu không chỉ định
            size = sub_task if sub_task else 'small'
            
            # Thử tải từ HuggingFace
            try:
                print(f"Đang tải dataset cho task refine, kích thước {size}...")
                dataset_name = 'tufts-nlp/code-refinement'
                
                # Tải dataset
                dataset = load_dataset(dataset_name, split=hf_split)
                
                # Lọc theo kích thước nếu cần
                if size == 'small':
                    # Lọc dataset để lấy các mẫu nhỏ
                    dataset = dataset.filter(lambda x: len(x['buggy']) < 300 and len(x['fixed']) < 300)
                
                inputs = dataset['buggy']
                outputs = dataset['fixed']
                
                print(f"Đã tải thành công {len(inputs)} mẫu cho task refine")
                return inputs, outputs, None
                ...
                file_prefix = os.path.join(data_dir, f"{split}_{size}" if size != 'small' else split)
                
                # Đọc dữ liệu từ tệp địa phương
                with open(f"{file_prefix}.buggy", 'r', encoding='utf-8') as f:
                    inputs = [line.strip() for line in f.readlines()]
                with open(f"{file_prefix}.fixed", 'r', encoding='utf-8') as f:
                    outputs = [line.strip() for line in f.readlines()]
            ...
            raise ValueError(f"Không thể tải dữ liệu cho task refine: {e}")
        ...
        try:
            ...
            # Thử tải từ HuggingFace
            try:
                print("Đang tải dataset cho task concode...")
                dataset_name = 'code_x_glue_tc_text_to_code'
                
                # Tải dataset
                dataset = load_dataset('microsoft/code_x_glue_tc_text_to_code', 'concode', split=hf_split)
                
                inputs = dataset['nl']
                outputs = dataset['code']
                
                print(f"Đã tải thành công {len(inputs)} mẫu cho task concode")
                return inputs, outputs, None
                ...
                # Đọc dữ liệu từ tệp địa phương
                with open(f"{file_prefix}.nl", 'r', encoding='utf-8') as f:
                    inputs = [line.strip() for line in f.readlines()]
                with open(f"{file_prefix}.code", 'r', encoding='utf-8') as f:
                    outputs = [line.strip() for line in f.readlines()]
            ...
            raise ValueError(f"Không thể tải dữ liệu cho task concode: {e}")
    
    # Các task khác nếu cần
    else:
        raise ValueError(f"Task không được hỗ trợ: {task}")
