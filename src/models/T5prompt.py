...
class PromptTuneT5(nn.Module):
    ...
    def add_task_tags(self, input_ids, task_name):
        ...
        tags = [f"<{splits[0]}>"]
        if len(splits) > 1:
            tags.append(f"<{splits[1]}>")
        else:
            tags.append("<extra>")

        task_tokens = self.tokenizer.encode(tags, return_tensors='pt')
    ...
    def get_scores(self, queries, keys, phase, type='cosine'):
        if type == 'cosine':
            ...
            cos_sim = torch.clamp(cos_sim, -1.0, 1.0)  # <== thêm dòng này
        else:
    ...
    def get_topk(self, similarity, scores, k, task_id, phase, batched=False):
        if batched:
            # Đảm bảo self.top_k không vượt quá kích thước của scores
            safe_top_k = min(self.top_k, scores.size(1))
            top_scores, top_indices = torch.topk(scores, k=safe_top_k, dim=1, sorted=True)
            batch_top_indices, batch_freq = torch.unique(top_indices.flatten(), return_counts=True)
            # Đảm bảo giá trị k cho topk thứ hai không vượt quá kích thước của batch_freq
            safe_k = min(self.top_k, batch_freq.size(0))
            _, ind = torch.topk(batch_freq, k=safe_k, sorted=True)
            ...
            if (self.args.pool_teacher == 'train' and phase == 'train') or self.args.pool_teacher == 'both':
                ...
                # Đảm bảo k không vượt quá kích thước của scores
                safe_k = min(k, scores.size(1))
                top_scores, top_indices = torch.topk(scores, k=safe_k, dim=1, sorted=True)
            top_vals = torch.cat([a[i].unsqueeze(0) for a, i in zip(similarity, top_indices)])
