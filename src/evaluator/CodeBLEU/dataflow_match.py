...
def corpus_dataflow_match(references, candidates, lang):
    try:
        ...
        LANGUAGE = Language(os.path.join(root_dir, 'parser/my-languages.so'), lang)
        parser = Parser()
        ...
        print('Warning: Language initialization failed, returning 0')
        return 0
    except Exception as e:
        print(f'Warning: Dataflow matching failed ({str(e)}), returning 0')
        return 0

    if total_count == 0:
        print("WARNING: No reference data-flows extracted, score degenerates to 0")
        return 0
